...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isServiceOrAnon = !authHeader || token === supabaseServiceKey || token === anonKey;
    if (authHeader && !isServiceOrAnon) {
      // [BUG FIX] Use getUser() instead of non-existent getClaims()
