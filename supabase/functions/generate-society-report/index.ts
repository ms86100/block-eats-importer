  ...
  try {
    ...
    for (const society of societies) {
      try {
        ...
        // Insert report
        const { error: reportInsertErr } = await supabase.from("society_reports").insert({
        ...
        if (reportInsertErr) {
          console.error(`Failed to insert report for society ${society.id}:`, reportInsertErr);
          continue;
        }
        ...
        if (members && members.length > 0) {
          ...
          for (let i = 0; i < notifications.length; i += 200) {
            const { error: insertErr } = await supabase.from("notification_queue").insert(notifications.slice(i, i + 200));
            if (insertErr) {
              console.error(`Failed to enqueue report notifications for society ${society.id}:`, insertErr);
            }
          }
        }
        ...
        console.error(`Failed to generate report for society ${society.id}:`, societyErr);
      }
    }

    // [BUG FIX] Trigger notification queue processor to deliver report notifications
